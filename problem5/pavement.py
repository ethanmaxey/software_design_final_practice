...
@task
def setup():
    sh('python -m pip install -U coverage parameterized')

@task
def test():
    sh('python -m coverage run --source src --omit "*/unixtime_gui.py" -m unittest discover -s test')
